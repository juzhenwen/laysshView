...
public class RoleInfServiceImpl extends BaseServiceImpl<TSysRoleInf> implements IRoleInfService {
	...
	public DatagridForLayUI datagrid(RoleInf roleInf, String insCd, boolean v) throws Exception {
		...
		if (!PbUtils.isEmpty(insCd)) {
			// sql +=
			// " and (c.uuid=:insCd or c.parent_ins_cd=:insCd or c.up_two_ins=:insCd or c.up_three_ins=:insCd)";
			// param.put("insCd", insCd);

			// 查找所有下一级 和 自己
			sql += " and (c.INS_DETAIL_NOT_MY like:insCd  or c.uuid=:insUuid)";
			param.put("insCd", "%%" + insCd + "%%");
			param.put("insUuid", insCd);
		}
	 ...
	 * @return
	 */
	private List<Map> getfunctionCds(String oper_cd) {
		String sql = "select distinct(t.function_cd) as functionCd from t_sys_role_function t where t.role_cd in(";
		sql += "select t.role_cd  from t_sys_role_oper t where t.oper_cd=:oper_cd )";
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("oper_cd", oper_cd);
	...
	public List<ZTree> getAllTreeByRose(String oper_cd) throws Exception {
		...
		List<Map> maps = getfunctionCds(oper_cd);

		String shql = "from TSysFunctionInf t ";
		if (maps != null && maps.size() > 0) {
			shql = "from TSysFunctionInf t where  functionCd in (";
			for (int i = 0; i < maps.size(); i++) {
				if (i > 0) {
					shql += ",";
				}
				shql += "'" + maps.get(i).get("functionCd").toString() + "'";
			}
			shql += ")";
		}
		...
		//菜单ID
		Map<String, String> menuTreesByRose=new HashMap<String, String>();

		if (sInfs != null && sInfs.size() > 0) {
			for (TSysFunctionInf tsFunctionInf : sInfs) {
				menuTreesByRose.put(tsFunctionInf.getTSysMenuInf().getId(),"1");
		...
		// List<ZTree> zTrees = getTrees();
		//List<ZTree> zTrees = menuTrees();
		List<ZTree> zTrees =menuTreesByRose(menuTreesByRose);
		for (ZTree z : zTrees) {
	 ...
	 * @throws Exception
	 */
	private List<ZTree> menuTreesByRose(Map<String, String> menuMaps) throws Exception {
		List<ZTree> trees = new ArrayList<ZTree>();
		String sql = "";
		// 总共最多支持6级
		for (int x = 0; x < 6; x++) {
		...
		if (menuInfs != null && menuInfs.size() > 0) {
			for (MenuInf t : menuInfs) {
				...
				//如果不是二级菜单,且没有在权限里面
				if(!PbUtils.isEmpty(t.getMenuUrl()) && !menuMaps.containsKey(t.getId())){
					continue;
				}
