	...
	<form name="form" class="layui-form" style="margin-top: 20px;" method="post" action="">
		...
		<div class="layui-form-item">
			...
			<label class="layui-form-label"  style="color:#F00">Java类名称</label>
				<div class="layui-input-block">
					<input type="text" id="className" name="className" maxlength="100" lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input" />
				</div>
		 </div>
		 ...
		 <div class="layui-form-item">
		  ...
		  <div class="layui-form-item">
		    ...
		    <div class="layui-input-block">
				<input type="text"  id="classPath"  name="classPath" value="" maxlength="200" lay-verify="required" placeholder="请输入" autocomplete="off" class="layui-input" />
			 </div>
		 </div>
		<div class="layui-form-item">
...
layui.use(['form', 'layedit', 'laydate','jquery'], function(){
  ...
  //初始化下拉框
  pbInitComCombox($,form,'codeGenerator/getTablesList','tableName','tableName','tableName');
  ...
  form.on('select(tableName)', function (data) {
      ...
      //字段集合
      var url='codeGenerator/getClassName?className='+tableName;
      $.ajax({
          ...
          success : function(data) {
              var dataValue=data.str;
              var className = dataValue.substring(0,1).toUpperCase()+dataValue.substring(1);
              $("#className").val(className);
              $("#classPath").val(data.path);
          }
      });
	  ...
	  //字段集合
	  url='codeGenerator/getFieldList?tableName='+tableName;
	  $.ajax({
			url : '<%=request.getContextPath()%>/'+url,
			type:'POST',//默认以get提交，以get提交如果是中文后台会出现乱码
			dataType : 'json',
			...
			success : function(data) {
				...
				for(var i=0;i<data.length;i++){
				 ...
				 //排序字段
			     $("#sortColumn").html("");
				 var sortUrl='codeGenerator/getFieldList?tableName='+tableName;
				 $.ajax({
   ...
   var index = layer.load(1);//开启进度条
    $.ajax({
	url : '<%=request.getContextPath()%>/codeGenerator/createCode',
			data : data.field,
			type : 'POST',//默认以get提交，以get提交如果是中文后台会出现乱码
			dataType : 'json',
