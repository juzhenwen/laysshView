...
<body>
	<form class="layui-form" style="margin-top: 20px;" method="post" action="">
		...
		<input name="insLvl" type="hidden" />
		<input name="insTp" type="hidden" />
		
		<div class="layui-form-item">
				<label class="layui-form-label" style="color:#F00">机构编号</label>
				<div class="layui-input-inline">
					<input type="text" name="insCd"  autocomplete="off" class="layui-input" />
				</div>
...
layui.use(['form', 'layedit', 'laydate','jquery'], function(){
  ...
  var data =JSON.parse(decodeURIComponent(getRequestParam().obj));
   ...
   var index = layer.load(1);//开启进度条
      $.ajax({
		url : '<%=request.getContextPath()%>/insInf/modify',
		data :data.field,
