...
<form class="layui-form"  style="margin-top: 20px;" method="post" action="">
  <div class="layui-form-item">
      ...
      <div class="layui-input-inline">
        <input type="text" name="operCd"  lay-verify="required" maxlength="10" autocomplete="off" class="layui-input" />
      </div>
      <label class="layui-form-label" style="color:#F00">用户状态</label>
      <div class="layui-input-inline">
        <select name="operSt" id="operSt"  lay-verify="required">
        </select>
    </div>
  ...
  <div class="layui-form-item">
   ...
   <div class="layui-form-item">
      ...
      <div class="layui-input-inline">
        <input type="text" value="初始密码默认:888888" readonly="readonly" autocomplete="off" class="layui-input" />
      </div>
  </div>
 <div class="layui-form-item">
...
layui.use(['form', 'layedit', 'laydate','jquery'], function(){
  ...
  //初始化岗位下拉框
  pbInitCombox($,form,'dictTypeCd=job_tp','job');
  
  //初始化用户状态下拉框
  pbInitCombox($,form,'dictTypeCd=oper_st','operSt');
  
  
  //获取所有的角色
  $.ajax({
		url : '<%=request.getContextPath()%>/roleInf/getList',
    ...
    form.on('submit(btnSubmit)', function(data){
       ...
       $.ajax({
			url : '<%=request.getContextPath()%>/operInf/add',
			data :data.field,
			type:'POST',//默认以get提交，以get提交如果是中文后台会出现乱码
			dataType : 'json',
