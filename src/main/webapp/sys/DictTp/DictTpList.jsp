...
<div class="layui-tab">
    <div class="layui-tab-content">
        <div class="layui-tab-item layui-show">
            <jsp:include page="DictTpSearch.jsp"></jsp:include>
        </div>
    </div>
</div>
...
<script>
    layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function(){
        ...
        table.render({
            elem: '#table' ,//table id
            url: '<%=request.getContextPath()%>/dictTp/datagrid',
            method : 'POST', //方式
            page : true,//是否开启分页
        ...
        function openAdd() {
            layer.open({
                ...
                area : [ '100%', '100%' ],
                boolean : true,
                content : [ 'DictTpAdd.jsp', 'yes'], //iframe的url，no代表不显示滚动条
        ...
        function openDetail(data) {
            layer.open({
                ...
                area : [ '100%', '100%' ],
                boolean : true,
                content : [ 'DictTpDetail.jsp?obj='+encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success : function(layero, lockIndex) {
                    var body = layer.getChildFrame('body', lockIndex);
                    //绑定解锁按钮的点击事件
                    body.find('button#close').on('click', function() {
        ...
        function openUpdate(data){
            layer.open({
                ...
                area: ['100%', '100%'],
                boolean:true,
                content: ['DictTpEdit.jsp?obj='+encodeURIComponent(JSON.stringify(data)), 'yes'], //iframe的url，no代表不显示滚动条
                success: function(layero, lockIndex) {
            ...
            layer.open({
                ...
                ,success: function(layero){
                    ...
                    btn.find('.layui-layer-btn0').on('click', function() {
                        ...
                        $.ajax({
                            url : '<%=request.getContextPath()%>/dictTp/remove',
                            data : {
                                dictTypeCd : ids
                            },
