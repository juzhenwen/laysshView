	...
	<form name="form" class="layui-form" enctype="multipart/form-data" style="margin-top: 20px;" method="post" action="">
		...
		<div class="layui-form-item">
			<div class="layui-inline">
				...
				<div class="layui-input-block">
					 <input type="hidden" name="archives" lay-verify="required"  id="archives" readOnly="readOnly"  placeholder="请输入" autocomplete="off" class="layui-input" />
				</div>
			</div>
...
layui.use(['form', 'layedit', 'laydate','jquery','upload'], function(){
  ...
  upload.render({
	    elem: '#test10'
	    ,url: '<%=request.getContextPath()%>/fileUpload/upload'
	    ,auto: false
	    //,multiple: true
  ...
  pbInitCombox($,form,'dictTypeCd=student_sex','sex',data.sex);//student_sex为数据字典类型编号
   ...
   var index = layer.load(1);//开启进度条
      $.ajax({
		url : '<%=request.getContextPath()%>/student/modify',
					data : data.field,
