...
public class ${className}Controller extends BaseController {
	...
	@ResponseBody
	public Json remove(@RequestParam(value = "${pkColumn}", required = true) String ${pkColumn}, HttpServletRequest request, HttpSession session) {
		Json j = new Json();
		try {
			${classNameToL}Service.remove(${pkColumn});
			j.setSuccess(true);
			j.setMsg("删除成功！");
		} catch (Exception e) {
			j.setSuccess(false);
		...
		//添加系统日志
		writeSysLog(j, ${pkColumn},request, session);
